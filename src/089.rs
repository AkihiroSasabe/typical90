...
fn main() {
    ...
    // [2]バブルソートの交換回数は、区間内の転倒数に等しい。
    // 閉区間[l,r]のバブルソートの回数制約は、閉区間[l,r]の転倒数の制約に置換可能。
    // [2-1] 転倒数の定義: 
    //     Ai > Aj かつi < jを満たす(i,j)の組の個数。つまり、「左にいるのに大きいやんけ」となる組の個数
    // [2-2]バブルソートの交換回数 == 転倒数 の証明:
    //     [2-2-1]隣り合う2要素を交換しても、転倒数が2以上落ちない。1落ちるか、0落ちる(変わらない)か
    //     [2-2-2]配列Aがソートされていなければ、Ai > Ai+1 となる要素iが1個以上存在し、
    //              それを交換すると転倒数が必ず1個落ちる
    //              例
    //                 2, 7, 3, 5: 転倒数2
    //                 2, 3, 7, 5: 転倒数1
    ...
    // 動的計画法の遷移. 計算量は愚直にやるとO(N^2)で、累積和を使うとO(N)で計算可能. 
